...
(() => {
  // --- Build alias + palette (defaults first, then registry merge) ---
  let EXPLICIT_ALIAS = {};
  ...
  let PALETTE = { ...DEFAULT_PALETTE }; // start with defaults
  try {
    ...
    if (regEl) {
      ...
      for (const slug in registry) {
        ...
        if (item.raw_names) item.raw_names.forEach(rn => { if (rn) EXPLICIT_ALIAS[rn] = slug; });
        if (item.class) EXPLICIT_ALIAS[item.class] = slug;
        if (item.palette) {
          // merge: defaults first, then registry overrides
            PALETTE[slug] = { ...(DEFAULT_PALETTE[slug]||{}), ...item.palette };
        }
      }
    }
  } catch(_) {}
  ...
  const endColor = cfg.end || cfg.start; // solid gradient if no end provided
  ...
  // mark any direct .category-title children to avoid fallback underline
  el.querySelectorAll('.category-title').forEach(t => t.classList.add('wrapped-by-theme'));
    });
    ...
    document.querySelectorAll('h1, h2, h3, span, div').forEach(el => {
      ...
      for (const cls of el.classList) {
        if (legacyClasses.includes(cls) || legacyClasses.includes(cls.replace(/-Post$/,''))) {
          ...
          // mark legacy element now wrapped
          wrapper.querySelectorAll('.category-title, h1, h2, h3').forEach(t => t.classList.add('wrapped-by-theme'));
          break;
        }
      }
    });
