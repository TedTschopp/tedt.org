...
Over the last thirty years, we optimized software around **human attention**:
...
seat-priced software**—is about to get devalued.
...
* more ways to take real action (tooling + permissions)
...
> A system that can observe, plan, decide, and execute multi-step work by calling tools/APIs, operating under policy constraints, producing an auditable record of actions and outcomes.
...
| **Operational coordination (in-flight state)** | **Operational state + event streams**       | In-flight work, queues, workflow state machines, triggers, reconciliation               |
| **Derived projections (contextual surfaces)**  | **Generated views + agent working context** | Fast task context, temporary summaries, “what matters right now,” ephemeral UI surfaces |
| **Step-level execution context**               | **Current task frame**                      | The specific request, constraints, approvals, and tool calls for *this* step            |
| **Reproducibility and traceability**           | **Traceability + replay + versioning**      | “What changed?”, “Which truth is current?”, “Can we reproduce what happened?”           |
| **Access boundaries and guardrails**           | **AuthZ + policy enforcement**              | Least privilege, scoped permissions, risk tiers, approvals, separation of duties        |
...
* **What did the agent see?** (inputs + sources + versions)
* **What did it do?** (tool calls + state changes)
* **Why did it do it?** (policy + reasoning summary + constraints)
...
2. IT creates a ticket in BMC Helix / SailPoint (or the employee fills a form).
3. IT checks policy docs, role definitions, and manager approval requirements.
4. IT logs into identity admin UI, searches the user, assigns a role/group.
5. IT updates the ticket, notifies manager, maybe attaches screenshots.
6. Security/audit later asks: “Why was this access granted? Who approved it?”

This is *all UI*.

It’s not “hard” work. It’s **coordination work**.

#### How it works in an agent-first model (API + policy + audit)

1. Employee asks in chat: “I need finance reporting access for Project X.”
2. The agent:

   * pulls the user’s org context (SucessFactors)
   * checks the access policy (policy-as-code)
   * finds the least-privilege role that satisfies the request
   * generates an approval request to the manager + data owner
...
#### What the “UI” becomes (synthetic receipt + approval surface)
...
* **Policy basis:** `FIN-ACCESS-042` (requires manager + data owner approval)
...
* **Evidence:** Links to the exact policy version + relevant tickets + trace log
* **Action:** Approve / Reject / Request exception
...
* volume of work (how many transactions/actions?)
* risk tier (what’s the blast radius?)
* audit/compliance level (what evidence must exist?)
...
* **task-level verbs** (not just create/update)
...
* **Distribution** (being embedded in the system of record / front door)
* **Trust + compliance + liability** (indemnities, auditability, safe defaults)
...
* “What version of policy/model/tools did it use?”
...
* prompt injection via tickets/docs/emails
...
That creates the new bottleneck of **coordination & control**, not compute.
