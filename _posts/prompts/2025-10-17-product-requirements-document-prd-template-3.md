...
prompt_content: |
  ...
  * Awareness of **dependencies or separate projects**: knowing if some features are being handled by other teams or future projects (so you can mark them out of scope here).
  ...
  ### Anti-Goals (Non-Goals / What We Intentionally Will Not Do)

  #### Purpose

  To explicitly state what this product is **not trying to achieve**, even if
  those things might sound valuable or are frequently requested. Anti-goals
  clarify the product’s intended role, prevent the solution from drifting into
  adjacent problem spaces, and help teams make consistent trade-offs when new
  ideas and edge cases emerge.

  Unlike “Out of Scope” items (which may be deferred to a later phase),
  **anti-goals are deliberate guardrails**: they define directions the product
  should **actively avoid** unless product strategy changes.

  #### Instructions

  * **Define “What we will not optimize for”:** List outcomes, directions, or
    product identities that the team should *not* pursue (e.g., “We will not
    attempt to replace System X” or “We will not optimize for highly customized
    workflows in v1”).
  * **Write them as clear, testable statements:** Use “We will not…” / “This
    product is not intended to…” phrasing. Anti-goals should be specific enough
    that the team can use them in decision-making.
  * **Differentiate from Out of Scope:**

    * **Out of Scope** = not included in this release, possibly later.
    * **Anti-goal** = not an intended direction for this product (not “later,” but
      “not this product”), unless strategy changes.
  * **Include a brief rationale (recommended):** Explain *why* it’s an anti-goal
    (e.g., “to avoid duplicating a system of record,” “to reduce operational
    complexity,” “to maintain security posture,” “to preserve usability for
    frontline staff”).
  * **Use anti-goals as a decision filter:** If a proposed feature conflicts with
    an anti-goal, it should trigger a discussion at the product strategy level
    (not a normal backlog trade-off).
  * Optionally, tie anti-goals to **guiding principles** or **North Star metrics**
    (“We will not pursue features that increase technician time-on-task without
  ...
  * **Anti-Goals:**
    ...
    * *Not a full system replacement* — The mobile app is **not intended to
      replace the Work Management System (WMS)**. The WMS remains the system of
      record; the app will focus on field execution and essential updates.
    * *Not a highly configurable platform in v1* — We will **not build a custom
      workflow builder** or region-by-region configurable screens for the initial
      release. Standardized workflows reduce training, support load, and
      operational variance.
    * *Not an analytics product* — We will **not pursue advanced analytics or
      predictive maintenance modeling inside the app**. Those capabilities belong
      in dedicated BI/analytics platforms and require separate data science and
      governance work.
    * *Not “offline-first for everything”* — We will **not guarantee full offline
      functionality across all features** (e.g., complex search, deep history). If
      offline support is needed, it will be intentionally scoped to specific
      critical tasks and clearly defined.
    * *Not a customer engagement channel* — We will **not add customer-facing
      communication, outage reporting, or customer self-service** into this
      product. The app’s primary users are internal field staff and supervisors.
    * *Not a data warehouse on a device* — We will **not store large volumes of
      operational or sensitive data locally** beyond what is required for the
      intended workflows (to minimize security risk and data staleness).

  #### Prerequisites

  * Clear alignment on **product vision, target users, and product “job to be
    done”** (so it’s obvious what the product should and should not become).
  * Agreement on **system boundaries and ownership** (e.g., which system is the
    system of record, which team owns workflow logic, where analytics lives).
  * Stakeholder input on “common assumption traps” — i.e., things stakeholders
    frequently expect that would pull the product in the wrong direction.

  #### Standards and Best Practices

  Anti-goals (often called **“Non-Goals”**) are a common best practice in PRDs,
  technical RFCs, and design docs because they make trade-offs explicit and reduce
  ambiguity during delivery. They help prevent “silent expansion” of product
  intent when new stakeholders join, requirements evolve, or edge cases
  accumulate.

  Strong anti-goals are:

  * **Strategy-aligned** (reinforce what the product is meant to be)
  * **Decision-enabling** (help teams say “no” consistently)
  * **Stable over time** (change only when product strategy changes—not with every
    sprint)

  If helpful, treat anti-goals as **guardrails**: any request that violates an
  anti-goal requires deliberate escalation and a documented strategy decision
  (rather than normal prioritization).
